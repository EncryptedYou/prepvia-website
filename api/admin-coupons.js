...
async function listCoupons() {
  ...
  const legacy = await getCoupon('PREPVIA50');
  if (legacy && !out.some(c => c.code === legacy.code)) out.push(legacy);
  return out;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (code === 'PREPVIA50') return res.status(400).json({ message: 'PREPVIA50 is reserved for the existing coupon.' });
    ...
    if (req.method === 'PATCH') {
      ...
      if (code === 'PREPVIA50') return res.status(400).json({ message: 'PREPVIA50 is managed as the existing legacy coupon.' });
    ...
    if (req.method === 'DELETE') {
      const code = cleanCode(req.body?.code);
      if (!code || code === 'PREPVIA50') return res.status(400).json({ message: 'This coupon cannot be deleted.' });
