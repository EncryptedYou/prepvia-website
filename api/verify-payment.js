...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // The signature proves the payment response was generated by Razorpay,
    // but also verify the payment object itself and bind it to this order.
    const paymentResponse = await fetch('https://api.razorpay.com/v1/payments/' + encodeURIComponent(payment_id), {
      headers: { Authorization: 'Basic ' + auth }
    });
    const razorpayPayment = await paymentResponse.json();
    if (!paymentResponse.ok)
      return res.status(400).json({ message: 'Unable to verify Razorpay payment.' });
    if (String(razorpayPayment.order_id || '') !== String(order_id) ||
        String(razorpayPayment.currency || '') !== 'INR' ||
        Number(razorpayPayment.amount) !== actualAmount ||
        String(razorpayPayment.status || '').toLowerCase() !== 'captured') {
      return res.status(400).json({ message: 'Payment is not valid or captured.' });
    }
