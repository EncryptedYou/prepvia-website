...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='PATCH'){
      ...
      if(action==='set_reward_override'){
        const hasPurchase=Object.prototype.hasOwnProperty.call(b,'purchase_reward_rupees');
        const hasView=Object.prototype.hasOwnProperty.call(b,'view_reward_rupees');
        if(!hasPurchase&&!hasView)return res.status(400).json({message:'Provide at least one earning value.'});
        if(hasPurchase){
          if(b.purchase_reward_rupees===null||String(b.purchase_reward_rupees).trim()==='')u.purchase_reward_override_paise=null;
          else {const n=Number(b.purchase_reward_rupees);if(!Number.isFinite(n)||n<0||n>100000)return res.status(400).json({message:'Invalid custom sale earning.'});u.purchase_reward_override_paise=Math.round(n*100)}
        }
        if(hasView){
          if(b.view_reward_rupees===null||String(b.view_reward_rupees).trim()==='')u.view_reward_override_paise=null;
          else {const n=Number(b.view_reward_rupees);if(!Number.isFinite(n)||n<0||n>100000)return res.status(400).json({message:'Invalid custom view earning.'});u.view_reward_override_paise=Math.round(n*100)}
        }
        u.earning_override_updated_at=Date.now();await redisSet('referral:user:'+id,JSON.stringify(u));
        return res.status(200).json({success:true,purchase_reward_rupees:u.purchase_reward_override_paise==null?null:u.purchase_reward_override_paise/100,view_reward_rupees:u.view_reward_override_paise==null?null:u.view_reward_override_paise/100});
      }
