...
async function recordVerifiedPurchase({order_id, payment_id, amount, coupon, referral_code, session_id, visitor_id, referrer, utm_source, utm_medium, utm_campaign, device}) {
  const pid = String(payment_id || "").trim().slice(0,120);
  if (!pid) throw new Error("Missing payment id for verified purchase.");
  const dedupeKey = "analytics:purchase:payment:" + pid;
  const claimed = await redis("set", dedupeKey, "1", "nx", "ex", 31536000);
  if (claimed && claimed.result !== "OK") return { duplicate: true };
  try {
    return await recordEvent("purchase", {
      ...
      data: { order_id:String(order_id||"").slice(0,120), payment_id:pid, amount:Number(amount||0), currency:"INR", coupon:coupon?String(coupon).slice(0,40):null, verified:true }
    });
  } catch (error) {
