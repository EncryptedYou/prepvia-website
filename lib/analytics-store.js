...
async function recordEvent(event, payload = {}) {
  ...
  // Page views are only valid for the homepage. The global dashboard counts
  // one homepage view per session, while referral analytics use a separate
  // referral+session key so a referral visit is never lost just because the
  // same browser session previously opened the homepage without a referral.
  let isNewGlobalPageView = true;
  let isNewReferralPageView = true;
  if (safe.event === "page_view") {
    ...
    if (safe.session_id) {
      ...
      isNewGlobalPageView = !(first && first.result !== "OK");
    } else {
      isNewGlobalPageView = true;
    }

    if (safe.referral_code && safe.session_id) {
      const referralViewKey = "analytics:referral_view:" +
        referralKeyPart(safe.referral_code) + ":" + safe.session_id.slice(0,100);
      const firstReferral = await redis("set", referralViewKey, "1", "nx", "ex", 2592000);
      isNewReferralPageView = !(firstReferral && firstReferral.result !== "OK");
    }
  }
  ...
  if (safe.referral_code) {
    ...
    if (safe.event === "page_view" && isNewReferralPageView) {
  ...
  if (safe.event === "page_view" && isNewGlobalPageView && safe.visitor_id) {
  ...
  if (safe.event !== "page_view" || isNewGlobalPageView) {
