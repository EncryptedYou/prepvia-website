...
async function createSession(user){const token=randomId()+randomId();await redisSet('referral:session:'+hash(token),JSON.stringify({userId:user.id,createdAt:Date.now()}),SESSION_TTL);return token;}
module.exports={REWARD_PAISE,DEFAULT_SETTINGS,getReferralSettings,getViewRatePaise,ATTRIBUTION_DAYS,SESSION_TTL,normalizeEmail,hash,hashPassword,verifyPassword,makeCode,safeUser,bearer,getSession,getUserById,getUserByCode,getUserByEmail,increment,setAdd,setCardinality,redisCommand,pushList,listRange,createNotification,createPendingEarning,createSession,redisGet,redisSet,redisSetNX,redisDel,randomId,cleanCode};
